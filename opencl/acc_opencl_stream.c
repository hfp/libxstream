...
#define ACC_OPENCL_CREATE_COMMAND_QUEUE(CTX, DEV, PROPS, RESULT)               \
  clCreateCommandQueue(                                                        \
      CTX, DEV,                                                                \
      (cl_command_queue_properties)(NULL != (PROPS) ? ((PROPS)[1]) : 0),       \
      RESULT)
#endif
...
c_dbcsr_acc_opencl_info_stream_t *c_dbcsr_acc_opencl_info_stream(void *stream) {
  assert(NULL == stream ||
         sizeof(c_dbcsr_acc_opencl_info_stream_t) <= (uintptr_t)stream);
  return (NULL != stream ? ((c_dbcsr_acc_opencl_info_stream_t
                                 *)((uintptr_t)stream -
                                    sizeof(c_dbcsr_acc_opencl_info_stream_t)))
                         : NULL);
}
...
int c_dbcsr_acc_stream_create(void **stream_p, const char *name, int priority) {
  ...
  c_dbcsr_timeset((const char **)&routine_name_ptr, &routine_name_len,
                  &routine_handle);
#endif
...
#else
  if (CL_QUEUE_PRIORITY_HIGH_KHR <= priority &&
...
#pragma omp critical(c_dbcsr_acc_opencl_stream)
    ...
    tid = (i < c_dbcsr_acc_opencl_config.nthreads
               ? i
               : (i % c_dbcsr_acc_opencl_config.nthreads));
    if (NULL !=
        c_dbcsr_acc_opencl_config
            .device) { /* inherit master's context if current context is NULL */
      LIBXSMM_ATOMIC_CMPSWP(
          &c_dbcsr_acc_opencl_config.device[tid].context, NULL,
          c_dbcsr_acc_opencl_config.device[/*main*/ 0].context,
          LIBXSMM_ATOMIC_RELAXED);
    }
  } else
    offset = c_dbcsr_acc_opencl_stream_counter_base++;
#endif
  if (NULL != c_dbcsr_acc_opencl_config.device)
    context = c_dbcsr_acc_opencl_config.device[tid].context;
  if (NULL != context) {
    cl_device_id device = NULL;
    result = clGetContextInfo(context, CL_CONTEXT_DEVICES, sizeof(cl_device_id),
                              &device, NULL);
    if (CL_SUCCESS == result) {
      ...
      if (0 == s || 0 == (tid % s)) {
        if (0 != c_dbcsr_acc_opencl_config.device[tid].intel) {
          const int xhints =
              ((1 == c_dbcsr_acc_opencl_config.xhints ||
                0 > c_dbcsr_acc_opencl_config.xhints)
          ...
          if (0 != (1 & xhints)) { /* attempt to enable command aggregation */
            ...
            const cl_command_queue q = ACC_OPENCL_CREATE_COMMAND_QUEUE(
                context, device, props, &result);
            if (CL_SUCCESS == result) {
              c_dbcsr_acc_opencl_config.timer =
                  c_dbcsr_acc_opencl_timer_host; /* force host-timer */
          ...
          if (0 != (2 & xhints)) { /* attempt to enable queue families */
            ...
            if (CL_SUCCESS ==
                clGetDeviceInfo(
                    device, 0x418B /*CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL*/,
                    sizeof(intel_qfprops), intel_qfprops, &nbytes)) {
              for (i = 0; (i * sizeof(*intel_qfprops)) < nbytes; ++i) {
                if (0 /*CL_QUEUE_DEFAULT_CAPABILITIES_INTEL*/ ==
                        intel_qfprops[i].capabilities &&
                    1 < intel_qfprops[i].count) {
        ...
        if ((c_dbcsr_acc_opencl_timer_device ==
             c_dbcsr_acc_opencl_config.timer) &&
            (3 <= c_dbcsr_acc_opencl_config.verbosity ||
             0 > c_dbcsr_acc_opencl_config.verbosity)) {
          properties[1] = CL_QUEUE_PROFILING_ENABLE;
        }
        queue = ACC_OPENCL_CREATE_COMMAND_QUEUE(context, device, properties,
                                                &result);
        ...
        for (i = 0; i < maxn; ++i) {
          ...
          if (0 < t && t != tid) { /* avoid cloning master's and own streams */
            streams = c_dbcsr_acc_opencl_config.streams +
                      ACC_OPENCL_STREAMS_MAXCOUNT * t;
        ...
        if (NULL != stream) { /* clone existing stream */
          ...
          for (i = 0; i < ACC_OPENCL_STREAMS_MAXCOUNT;
               ++i) { /* adopt master's stream created last */
  ...
  if (EXIT_SUCCESS == result) {
    ...
    if (i < ACC_OPENCL_STREAMS_MAXCOUNT) { /* register stream */
      ...
      const size_t size =
          sizeof(cl_command_queue) + sizeof(void *) + size_info - 1;
      void *const handle = malloc(size);
      assert(NULL != queue);
      if (NULL != handle) {
        const uintptr_t address = (uintptr_t)handle;
        const uintptr_t aligned =
            LIBXSMM_UP2(address + size_info, sizeof(void *));
...
#pragma omp critical(c_dbcsr_acc_opencl_stream)
  ...
  if (NULL != stream) {
      ...
      for (; tid < c_dbcsr_acc_opencl_config.nthreads; ++tid) { /* unregister */
        ...
        for (i = 0; i < ACC_OPENCL_STREAMS_MAXCOUNT; ++i) {
          if (stream == streams[i]) {
            const int j = i + 1,
                      result_release =
                          clReleaseCommandQueue(queue); /* soft-error */
            if (j < ACC_OPENCL_STREAMS_MAXCOUNT &&
                NULL !=
                    streams[j]) { /* compacting streams is not thread-safe */
              memmove(streams + i, streams + j,
                      sizeof(void *) * (ACC_OPENCL_STREAMS_MAXCOUNT - j));
            }
    ...
    c_dbcsr_acc_opencl_stream_counter_base = c_dbcsr_acc_opencl_stream_counter =
        0; /* reset */
...
int c_dbcsr_acc_stream_priority_range(int *least, int *greatest) {
  int result =
      ((NULL != least || NULL != greatest) ? EXIT_SUCCESS : EXIT_FAILURE);
...
int c_dbcsr_acc_stream_sync(void *stream) {
  ...
  const int *const priority =
      (0 == (1 & c_dbcsr_acc_opencl_config.flush)
           ? NULL
           : c_dbcsr_acc_opencl_stream_priority(stream));
#endif
#if defined(__DBCSR_ACC) && defined(ACC_OPENCL_PROFILE)
  int routine_handle;
  static const char *const routine_name_ptr = LIBXSMM_FUNCNAME;
  ...
  assert(NULL != stream);
#if defined(ACC_OPENCL_STREAM_PRIORITIES)
  if (NULL != priority && CL_QUEUE_PRIORITY_HIGH_KHR <= *priority &&
