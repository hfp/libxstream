...
******************************************************************************/
/**
 * Ozaki scheme 2 through the host shim (see shim_ozaki2.h): fp64 with symmetric
 * residues (OZAKI_SYMRES), the signed datapath that keeps |r| <= m/2 over the
 * same moduli table. The tables are the ones ozaki_crt_moduli_flags emitted for
 * 16 moduli and OZAKI_SYMRES=1, captured from a run rather than derived again
 * here, so the kernels are held to what the host actually builds.
 */
#define USE_DOUBLE 1
#define MANT_BITS 52
#define KGROUPS 0
#define OZAKI_UNFUSE 1
#define OZAKI_SYMRES 1
#define NMODULI 16
...
int main(void)
{
  return shim_ozaki2("fp64 symres");
}
