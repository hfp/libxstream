...
******************************************************************************/
/**
 * Ozaki scheme 2 through the host shim (see shim_ozaki2.h): fp64 with a Barrett
 * reduction every KGROUPS*BK steps of K, which is what keeps a K past the
 * accumulator window exact (OZAKI_GROUPS=2 requests it; a declared K past the
 * window derives it). Two groups reduce every 64 steps, so the shapes here
 * cross several reductions where the window itself would take a K of 32768. The
 * tables are the ones ozaki_crt_moduli_flags emitted for 16 moduli and
 * OZAKI_GROUPS=2, captured from a run rather than derived again here, so the
 * kernels are held to what the host actually builds.
 */
#define USE_DOUBLE 1
#define MANT_BITS 52
#define KGROUPS 2
...
int main(void)
{
  return shim_ozaki2("fp64 kgroups=2");
}
