  ...
  int ntasks, int param_format, int stop_at_impure)
{
  ...
  if (0 == param_format) { /* native */
    ...
    info->max_n = first_n;
    info->max_k = first_k;
    for (info->mnk_changes = 0; 0 != result && i < (ntasks * stride); i += stride) {
  ...
  const double* pack_b_data, double* shard_c_data)
{
  ...
  const int verbosity = config->verbosity, trace = (0 > verbosity || 2 < verbosity);
  ...
  int task_complete = 0;
  LIBXS_ASSERT(NULL != pack_a_data && NULL != pack_b_data && NULL != shard_c_data);
  ...
  if (0 < ntasks) {
    ...
    { /* base init state: computed once, shared across all specializations */
      ...
      libxs_lock_t* const lock_memory =
        (NULL != devinfo->clSetKernelArgMemPointerINTEL ? NULL : config->lock_memory);
      libxstream_opencl_info_memptr_t adata, bdata, cdata, batch;
      const int stride = (0 == param_format ? DBM_OPENCL_TASK_SIZE : 3);
      size_t work_size[] = {1, 1, 1}, ibatch = 0;
      const size_t work_tasks = ntasks;
      cl_kernel kernel = NULL;
      int bk, bk0, use_blkrd = 0;
      cl_int size;
      LIBXS_ASSERT(NULL != str && NULL != str->queue);
      ...
      if (0 == LIBXS_ATOMIC_LOAD(&base_ready, LIBXS_ATOMIC_SEQ_CST)) {
        ...
        if (0 == base_ready) {
          ...
          offset += (size_t)libxstream_opencl_flags_atomics(devinfo,
            libxstream_opencl_atomic_fp_64, base_exts, &base_nexts, base_flags + offset,
          ...
          wgsize[0] = (NULL == wg_env ? LIBXS_MAX((unsigned long int)sm, devinfo->wgsize[1])
                                      : strtoul(wg_env, NULL, 10));
          if (1 < sgsize && 0 < wgsize[0]) { /* subgroups */
            if (LIBXS_DELTA(wgsize[0], devinfo->wgsize[1]) <= LIBXS_DELTA(wgsize[0], sgsize)) {
          ...
          { /* 256-GRF */
            ...
            const size_t max_wgs = (0 != biggrf) ? devinfo->wgsize[0] / 2 : devinfo->wgsize[0];
          ...
          if (0 != nz) {
            offset += (size_t)LIBXS_SNPRINTF(
              base_flags + offset, sizeof(base_flags) - offset, " -DNZ=%i", nz);
          }
          sgsize_s = sgsize;
          base_source = source;
      ...
      { /* assume A, B, and C do not carry an offset */
        ...
        result |= libxstream_opencl_info_devptr_lock(
          &adata, NULL /*lock*/, pack_a_data, 1 /*esize*/, NULL /*amount*/, &iadata);
        result |= libxstream_opencl_info_devptr_lock(
          &bdata, NULL /*lock*/, pack_b_data, 1 /*esize*/, NULL /*amount*/, &ibdata);
        result |= libxstream_opencl_info_devptr_lock(
          &cdata, NULL /*lock*/, shard_c_data, 1 /*esize*/, NULL /*amount*/, &icdata);
        LIBXS_ASSERT(0 == iadata && 0 == ibdata && 0 == icdata);
      }
      result |= libxstream_opencl_info_devptr_lock(
        &batch, NULL /*lock*/, params /*batch*/, sizeof(int) * stride, &work_tasks, &ibatch);
      if (NULL != lock_memory) {
      ...
      result |= opencl_libsmm_acc_process(params_host, params, ntasks,
        DBM_OPENCL_DBCSR_TYPE_REAL_8, pack_a_data, pack_b_data, shard_c_data, task.max_m,
        task.max_n, task.max_k, dbm_multiply_opencl_smm, 1 /*homogeneous*/, stream,
        NULL /*c_stream*/,
