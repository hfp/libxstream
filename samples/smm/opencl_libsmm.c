...
#   define LIBXSTREAM_CHECK(RESULT, EXPR, MSG) do { \
...
# endif
        ...
        if (NULL != env_params && '\0' != *env_params) { /* filename */
            ...
            if (NULL != fgets(buffer, LIBXSTREAM_BUFFERSIZE, file)) {
              ...
              while (NULL != fgets(buffer, LIBXSTREAM_BUFFERSIZE, file)) { /* read params from CSV-file */
                if (EXIT_SUCCESS == opencl_libsmm_read_smm_params(buffer, &key, &config, gflops, device, NULL /*key_ok*/)) {
                  ...
                  key.devuid = 0;
                  config_init = (opencl_libsmm_smm_t*)libxs_registry_get(opencl_libsmm_registry, &key, sizeof(key), NULL);
                  if (NULL == config_init) {
                    if (NULL == libxs_registry_set(opencl_libsmm_registry, &key, sizeof(key), &config, sizeof(config), NULL)) {
                      result = EXIT_FAILURE;
                      break;
                    }
                    else ++ntuned;
          ...
          { /* try OPENCL_LIBSMM_SMM_PARAMS as string of kernel parameters (not device-specific) */
            ...
            if (NULL != libxs_registry_set(opencl_libsmm_registry, &key_direct, sizeof(key_direct), &config, sizeof(config), NULL)) {
              libxstream_opencl_config.devmatch = 0; /* disable device-match */
              ntuned = 1;
            }
            else result = EXIT_FAILURE;
            ...
            if (NULL != next && next < (line + LIBXSTREAM_BUFFERSIZE)) {
                                    ...
                                    buffer, &key, &config, &perfest, bufname /*consume name/id*/, NULL /*key_ok*/))
              {
                if (0 == key_direct_skip || 0 != memcmp(&key_direct, &key, (const char*)&key.k - (const char*)&key)) {
                  ...
                  config_init = (opencl_libsmm_smm_t*)libxs_registry_get(opencl_libsmm_registry, &key, sizeof(key), NULL); /* duplicate? */
                  if (NULL == config_init) {
                    if (NULL != libxs_registry_set(opencl_libsmm_registry, &key, sizeof(key), &config, sizeof(config), NULL)) ++ntuned;
                  ...
                  if (active_match == i && 0 != default_uid) {
                    ...
                    config_init = (opencl_libsmm_smm_t*)libxs_registry_get(opencl_libsmm_registry, &key, sizeof(key), NULL);
                    if (NULL != config_init || NULL != libxs_registry_set(opencl_libsmm_registry, &key, sizeof(key), &config, sizeof(config), NULL)) {
                      static int info = 0;
...
# endif
      ...
      mn <= (max_kernel_dim * max_kernel_dim))
  {
    ...
    config = (opencl_libsmm_trans_t*)libxs_registry_get(opencl_libsmm_registry, &key, sizeof(key), NULL);
    if (NULL == config) {
            ...
            if (EXIT_SUCCESS == result) {
              ...
              if (wgsize_max < new_config.wgsize) {
                ...
                if (0 < nchar && (int)sizeof(build_params) > nchar) {
                  result = libxstream_opencl_kernel(0 /*source_kind*/, OPENCL_KERNELS_SOURCE_TRANSPOSE, fname, build_params,
                    buffer, NULL /*try*/, NULL /*try_ok*/, NULL /*extnames*/, 0 /*num_exts*/, &new_config.kernel);
                }
                else result = EXIT_FAILURE;
              }
              if (EXIT_SUCCESS == result) {
                config = (opencl_libsmm_trans_t*)libxs_registry_set(opencl_libsmm_registry, &key, sizeof(key), &new_config, sizeof(new_config), NULL);
...
# endif
      ...
      config = (opencl_libsmm_smm_t*)libxs_registry_get(opencl_libsmm_registry, &key, sizeof(key), NULL);
...
# endif
        ...
        if (EXIT_SUCCESS == result) {
          ...
          if (NULL != tname) {
            ...
            new_config.ws = (0 >= wgmin ? (0 == kernel_idx ? (0 != defaults ? LIBXS_MAX(m_max, n_max)
...
# endif
            ...
            if (new_config.wgsize[kernel_idx] <= devinfo->wgsize[0]) { /* SMM can be handled by device */
              ...
              nchar = LIBXS_SNPRINTF(build_params, sizeof(build_params),
                ...
                tname, CL_DEVICE_TYPE_GPU == devinfo->type, cmem, LIBXS_CAST_INT(new_config.wgsize[kernel_idx]), LIBXS_CAST_INT(sgs), fname,
              ...
              if (EXIT_SUCCESS == result) {
                ...
                if (EXIT_SUCCESS == result) {
                  ...
                  if (new_config.wgsize[kernel_idx] <= wgsize_max_kernel) { /* check planned WG-size vs kernel-specific WG-size */
                    if (NULL == config || NULL == config->kernel[kernel_idx]) {
                      config = (opencl_libsmm_smm_t*)libxs_registry_set(opencl_libsmm_registry, &key, sizeof(key), &new_config, sizeof(new_config), NULL);
                    }
                    if (NULL != config) {
                      if (2 <= libxstream_opencl_config.verbosity || 0 > libxstream_opencl_config.verbosity) {
                        const double duration = libxs_timer_duration(start, libxs_timer_tick());
...
# endif
        ...
        if (EXIT_SUCCESS != result && NULL != config) {
          libxs_registry_remove(opencl_libsmm_registry, &key, sizeof(key), NULL);
        }
# if defined(__DBCSR_ACC)
        c_dbcsr_timestop(&routine_handle);
# endif
