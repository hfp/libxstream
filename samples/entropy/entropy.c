...
# endif
  ...
  if (0 <= entropy && 8 >= entropy) {
    if ((1 << 20) <= nitems) { /*mega*/
      fprintf(stdout, "Compression %gx: %.1f -> %.1f MB", 0 != entropy ? (8.0 / entropy) : 1.0, mega * nitems, mega * entropy * nitems / 8.0);
    }
    else if ((1 << 10) <= nitems) { /*kilo*/
      fprintf(stdout, "Compression %gx: %.1f -> %.1f KB", 0 != entropy ? (8.0 / entropy) : 1.0, kilo * nitems, kilo * entropy * nitems / 8.0);
    }
    else  {
      fprintf(stdout, "Compression %gx: %.0f -> %0.f B", 0 != entropy ? (8.0 / entropy) : 1.0, 1.0 * nitems, entropy * nitems / 8.0);
    }
